...
public class AdminController {
    ...
    @GetMapping("/profile")
    public ResponseEntity<MemberInfoResponse> getMyInfo(@AuthenticationPrincipal Admin admin) {
        if (admin == null) {
            return ResponseEntity.status(401).build(); // 인증 필요
        }

        // loginId 기준으로 안전하게 조회
        MemberInfoResponse response = adminService.getAdminInfo(admin.getLoginId());
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<MemberApartmentResponse> getMyApartmentInfo(@AuthenticationPrincipal Admin admin) {
        if (admin == null) {
            return ResponseEntity.status(401).build(); // 인증 필요
        }

        // loginId 기준으로 안전하게 조회
        MemberApartmentResponse response = adminService.getAdminApartmentInfo(admin.getLoginId());
        return ResponseEntity.ok(response);
    }
}
